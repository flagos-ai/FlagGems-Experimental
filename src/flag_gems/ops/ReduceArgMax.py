...
@triton.jit
def reduce_argmax_kernel_non_inner(
    inp,
    out_index,
    M,
    N,
...
@triton.jit
def reduce_argmax_kernel_inner(
    inp,
    out_index,
    M,
    N,
...
def ReduceArgMax(inp, dim=None, keepdim=False, *, dtype=None):
    logger.debug("GEMS REDUCE_ARGMAX")
    if dim is None:
        M = inp.numel()
        if dtype is None:
            dtype = inp.dtype
        ...
        with torch_device_fn.device(inp.device):
            reduce_argmax_kernel_1[(mid_size, 1, 1)](
                inp,
                mid_value,
                mid_index,
                M,
        ...
        with torch_device_fn.device(inp.device):
            if K > 1:
                ...
                reduce_argmax_kernel_non_inner[grid](
                    inp,
                    out_index,
                    M,
                    N,
                ...
                reduce_argmax_kernel_inner[grid](
                    inp,
                    out_index,
                    M,
                    N,
