...
# Quantized tensors carry their own backend key. The autogenerated `.out`
# overloads of quantized-producing ops are composites that resolve at the
# quantized key (a plain backend-key registration is never reached for them,
# verified with a per-key sentinel), so they need this extra key -- the same
# trap as the sparse keys above.
...
_FULL_CONFIG = (
    ...
    (
        "_make_per_channel_quantized_tensor.out",
