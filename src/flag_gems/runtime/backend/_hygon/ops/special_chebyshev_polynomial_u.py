...
# ---------------------------------------------------------------------------
@triton.jit
def _cheb_u_scalar_kernel(x_ptr, out_ptr, numel,
                          CAST_F32: tl.constexpr, N: tl.constexpr,
                          BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < numel
    x = tl.load(x_ptr + offs, mask=mask, cache_modifier=".cg", eviction_policy="evict_first")
    if CAST_F32:
        x = x.to(tl.float32)
    if N == 0:
        r = x - x + 1.0
...
@triton.jit
def _cheb_u_tensor_kernel(x_ptr, n_ptr, out_ptr, numel,
                          CAST_F32: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < numel
...
@triton.jit
def _cheb_u_bcast_kernel(x_ptr, n_ptr, out_ptr, numel,
...
def _run_tensor(x, n):
    ...
    _cheb_u_minmax_kernel[grid_m](
        n, max_buf, min_buf, n.numel(), BLOCK=_BLOCK)
    ...
    if sx == sn:
        ...
        cast_f32 = not x.dtype.is_floating_point
        grid = (triton.cdiv(numel, _BLOCK),)
        _cheb_u_tensor_kernel[grid](
            x, n, out, numel, CAST_F32=cast_f32, BLOCK=_BLOCK)
        return out

    out_shape, xb, nb, nd = _broadcast_meta(sx, sn, tuple(x.stride()), tuple(n.stride()))
    numel = 1
    for s in out_shape:
        numel *= s
    ...
    _cheb_u_bcast_kernel[grid](
        x, n, out, numel,
        s_pad[0], s_pad[1], s_pad[2], s_pad[3],
        s_pad[4], s_pad[5], s_pad[6], s_pad[7],
        xb_pad[0], xb_pad[1], xb_pad[2], xb_pad[3],
        xb_pad[4], xb_pad[5], xb_pad[6], xb_pad[7],
        nb_pad[0], nb_pad[1], nb_pad[2], nb_pad[3],
        nb_pad[4], nb_pad[5], nb_pad[6], nb_pad[7],
        CAST_F32=cast_f32, BLOCK=_BLOCK)
    return out
