...
def _addmm_even(
    ...
    self_ptrs = self_ptr + offs_m[:, None] * stride_self_m + offs_n[None, :] * stride_self_n
    s = tl.load(self_ptrs)
...
def run(self, mat1, mat2, *, beta=1, alpha=1):
    ...
    if even:
        _addmm_even[grid](
            ...
            num_warps=WARPS, num_stages=STAGES, sched_latency="mmac5-ds6",
        )
    else:
        _addmm_masked[grid](
            self, mat1, mat2,
