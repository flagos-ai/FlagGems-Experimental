...
def run(x, n):
    ...
    if tensor_mode:
        ...
        out_dtype = torch.promote_types(xdt, n.dtype) if n.dtype.is_floating_point else xdt
        _log(f"[legendre] tensor-n x={tuple(x.shape)}/{xdt} n={tuple(n.shape)}/{n.dtype} "
        ...
        _log(f"[legendre] scalar-n x={tuple(x.shape)}/{xdt} n={nv} type={type(n).__name__}")
        nv_scalar = nv
    ...
    if (not tensor_mode) and x.is_contiguous():
        ...
        _legendre_flat_kernel[grid](
            x, out, numel, nv_scalar, EVEN=(numel % BLOCK == 0),
    ...
    _legendre_kernel[grid](
        x, n if tensor_mode else x, out, numel, nv_scalar,
        s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7],
        xs[0], xs[1], xs[2], xs[3], xs[4], xs[5], xs[6], xs[7],
        ns[0], ns[1], ns[2], ns[3], ns[4], ns[5], ns[6], ns[7],
        NDIM=ndim, SCALAR_N=(not tensor_mode), FLAT=flat, BLOCK=BLOCK,
        num_warps=warps,
    )
    return out
