...
def run(grad, indices, offset2bag, bag_size, maximum_indices, num_weights,
        scale_grad_by_freq, mode, per_sample_weights=None, padding_idx=-1):
    if mode == 2:
        raise RuntimeError(
    ...
    _emb_bag_dense_bwd_flat_kernel[grid](
        grad, indices, offset2bag, bag_size, dummy, acc,
        N, D, padding_idx,
        mode == 1, scale_grad_by_freq, per_sample_weights is not None,
        is_lowp, d_pow2, d_shift, d_mask, BLOCK=BLOCK, num_warps=4,
    )

    if is_lowp:
        n_el = num_weights * D
        _cast_kernel[(triton.cdiv(n_el, 2048),)](
            acc, out, n_el, BLOCK=2048,
        )
