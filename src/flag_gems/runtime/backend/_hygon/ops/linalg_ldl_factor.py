...
def _ldl_factor_kernel(
    ...
    stride_pb,
    BLOCK_N: tl.constexpr, NB: tl.constexpr, KC: tl.constexpr,
):
    ...
    # LD = (A + A^T) / 2
    a_ij = tl.load(a_base + rows[:, None] * stride_am + cols[None, :] * stride_an,
                   mask=rmask[:, None] & cmask[None, :], other=0.0)
    a_ji = tl.load(a_base + cols[None, :] * stride_am + rows[:, None] * stride_an,
                   mask=rmask[:, None] & cmask[None, :], other=0.0)
    tl.store(l_base + rows[:, None] * stride_lm + cols[None, :] * stride_ln,
             (a_ij + a_ji) * 0.5, mask=rmask[:, None] & cmask[None, :])
    ...
    for kb in tl.range(0, n, NB, num_stages=2):
        ...
        panel = tl.load(l_base + p_rows[:, None] * stride_lm + pcols[None, :] * stride_ln,
                        mask=p_mask[:, None] & (pcols[None, :] < kb_end), other=0.0)
        ...
        while jc < kb:
            j = jc + tl.arange(0, KC)
            jmask = j < kb
            tile1 = tl.load(l_base + p_rows[:, None] * stride_lm + j[None, :] * stride_ln,
                            mask=p_mask[:, None] & jmask[None, :], other=0.0)
            tile2 = tl.load(l_base + pcols[:, None] * stride_lm + j[None, :] * stride_ln,
                            mask=(pcols[:, None] < kb_end) & jmask[None, :], other=0.0)
            d_j = tl.load(l_base + j * stride_lm + j * stride_ln, mask=jmask, other=0.0)
            panel -= tl.dot(tile1, tl.trans(tile2 * d_j[None, :]), input_precision="ieee")
            jc += KC

        # Block-diagonal D values (first NB entries).
        d_block = tl.sum(tl.where(rowidx[:, None] == colidx[None, :], panel, 0.0), axis=0)
        ...
        for kk in range(kb, kb_end):
            ...
            dsum = tl.sum(tl.where(colidx < c, (row_kk * row_kk) * d_block, 0.0), axis=0)
            d_kk = s_kk - dsum
            new_col = (raw_col - acc_i) / d_kk
            panel = tl.where(colidx[None, :] == c, new_col[:, None], panel)
            panel = tl.where((rowidx[:, None] == c) & (colidx[None, :] == c), d_kk, panel)
            d_block = tl.where(colidx == c, d_kk, d_block)

        # Store the factored panel.
        tl.store(l_base + p_rows[:, None] * stride_lm + pcols[None, :] * stride_ln, panel,
                 mask=p_mask[:, None] & (pcols[None, :] < kb_end))

    # Zero the upper triangle (i < j).
    tl.store(l_base + rows[:, None] * stride_lm + cols[None, :] * stride_ln, 0.0,
...
def run(self, *, hermitian=False):
    ...
    _ldl_factor_kernel[grid](
        ...
        LD.stride(-2), LD.stride(-1),
        pivots.stride(0) if pivots.ndim > 1 else 0,
        BLOCK_N=BLOCK_N, NB=NB, KC=KC,
    )
    return (LD, pivots)
