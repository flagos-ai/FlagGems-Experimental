...
@triton.jit
def _beam_search_score_2d(log_probs, beam_scores, out, vocab_size, row_stride,
                          EVEN: tl.constexpr, BLOCK_V: tl.constexpr):
...
@triton.jit
def _beam_search_score_flat(log_probs, beam_scores, out, vocab_size, num_rows,
                            EVEN: tl.constexpr, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
...
def run(log_probs, beam_scores):
    ...
    if numel >= _BIG_NUMEL:
        ...
        _beam_search_score_flat[grid](
            log_probs, bs, out, last_dim, num_rows,
            EVEN=even, BLOCK=_BLOCK_FLAT, num_warps=_NUM_WARPS_FLAT,
        )
    else:
        ...
        if numel >= _MED_NUMEL:
            ...
            _beam_search_score_2d[grid](
                log_probs, bs, out, last_dim, row_stride,
                EVEN=even, BLOCK_V=_BLOCK_2D_MED, num_warps=_NUM_WARPS_2D_MED,
            )
        else:
            ...
            _beam_search_score_2d[grid](
                log_probs, bs, out, last_dim, row_stride,
                EVEN=even, BLOCK_V=_BLOCK_2D_SMALL, num_warps=_NUM_WARPS_2D_SMALL,
            )
    return out
