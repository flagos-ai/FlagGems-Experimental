...
@triton.jit
def _scatter_kernel(src_ptr, out_ptr,
                    n_diag, diag_len, offset,
                    st1, st2, src_st_t,
                    bp0, sz0, out_st0, src_st0,
                    bp1, sz1, out_st1, src_st1,
                    bp2, sz2, out_st2, src_st2,
                    bp3, sz3, out_st3, src_st3,
                    bp4, sz4, out_st4, src_st4,
                    bp5, sz5, out_st5, src_st5,
                    bp6, sz6, out_st6, src_st6,
                    bp7, sz7, out_st7, src_st7,
...
@triton.jit
def _fused_kernel(in_ptr, src_ptr, out_ptr, numel,
                  log2_s1, log2_s2, s1_m1, s2_m1, s2,
                  offset, diag_len,
...
def _try_fused(input, src, out, offset, d1, d2, ndim, sizes, diag_len):
    ...
    _fused_kernel[(triton.cdiv(numel, BLOCK_FUSED),)](
        input, src, out, numel,
        log2_s1, log2_s2, s1 - 1, s2 - 1, s2,
        offset, diag_len,
        SRC_MODE=mode, BLOCK=BLOCK_FUSED, num_warps=4)
    return True
...
def run(input, src, offset=0, dim1=0, dim2=1):
    ...
    if diag_len > 0:
        ...
        _scatter_kernel[(triton.cdiv(n_diag, BLOCK_SCATTER),)](
            ...
            st1, st2, src_st_t,
            *args,
            N_BATCH=m, BLOCK=BLOCK_SCATTER, num_warps=4)
    return out
