...
def _upsample_nearest_exact2d_backward_kernel(
    ...
    if FAST:
        ...
        c2 = tl.arange(0, 2)
        if IS_F64:
            r0 = tl.load(grad_out_ptr + base[:, None] + c2[None, :],
                         mask=valid[:, None], other=0.0).to(tl.float64)
            r1 = tl.load(grad_out_ptr + base[:, None] + g_s2 + c2[None, :],
                         mask=valid[:, None], other=0.0).to(tl.float64)
...
def run(grad_output, output_size, input_size, scales_h=None, scales_w=None):
    ...
    if scales_h is not None and scales_h > 0:
        ...
        scale_h = (np.float64(H_in) / np.float64(H_out)) if is_f64 else float(
            np.float32(H_in) / np.float32(H_out))
    if scales_w is not None and scales_w > 0:
        rw = 1.0 / float(scales_w)
        scale_w = np.float64(rw) if is_f64 else float(np.float32(rw))
    else:
        scale_w = (np.float64(W_in) / np.float64(W_out)) if is_f64 else float(
            np.float32(W_in) / np.float32(W_out))

    fast = (
        scale_h == 0.5 and scale_w == 0.5
        and H_out == 2 * H_in and W_out == 2 * W_in
    )
    ...
    _upsample_nearest_exact2d_backward_kernel[grid](
        ...
        C, total, H_in, W_in, H_out, W_out,
        scale_h, scale_w,
        BLOCK=BLOCK, CHUNK=CHUNK, NCHUNK=NCHUNK, IS_F64=is_f64, NSEARCH=32,
        FAST=fast,
        num_warps=4,
    )
    return out
