...
def _ormqr_kernel(
    ...
    DIM, F, K,
    stride_im, stride_ik,          # input strides: reflector axis, k
    stride_ib,                     # input batch stride
    stride_tb,                     # tau batch stride
    stride_c0, stride_c1,          # C (other/out) strides: tile axis0, tile axis1
    stride_cb,                     # C batch stride
    LEFT: tl.constexpr, ASCEND: tl.constexpr,
    ...
    for r in range(0, DIM, BLOCK_R):
        ...
        rmask = off_r < DIM
        if LEFT:
            tile = tl.load(base_oth + off_r[:, None] * stride_c0 + offs_f[None, :] * stride_c1,
                           mask=rmask[:, None] & fmask[None, :], other=0.0)
            tl.store(base_out + off_r[:, None] * stride_c0 + offs_f[None, :] * stride_c1,
                     tile, mask=rmask[:, None] & fmask[None, :])
        else:
            tile = tl.load(base_oth + offs_f[:, None] * stride_c0 + off_r[None, :] * stride_c1,
                           mask=fmask[:, None] & rmask[None, :], other=0.0)
            tl.store(base_out + offs_f[:, None] * stride_c0 + off_r[None, :] * stride_c1,
                     tile, mask=fmask[:, None] & rmask[None, :])
    ...
    for j in range(0, K):
        ...
        if LEFT:
            ...
            while r < DIM:
                off_r = r + tl.arange(0, BLOCK_R)
                rmask = off_r < DIM
                v = tl.load(base_in + off_r * stride_im + i * stride_ik, mask=rmask, other=0.0)
                v = tl.where(off_r == i, 1.0, v)
                ctile = tl.load(base_out + off_r[:, None] * stride_c0 + offs_f[None, :] * stride_c1,
                                mask=rmask[:, None] & fmask[None, :], other=0.0)
            ...
            while r < DIM:
                ...
                tl.store(base_out + off_r[:, None] * stride_c0 + offs_f[None, :] * stride_c1,
                         ctile, mask=rmask[:, None] & fmask[None, :])
                r += BLOCK_R
        else:
            ...
            while r < DIM:
                ...
                v = tl.load(base_in + off_r * stride_im + i * stride_ik, mask=rmask, other=0.0)
                v = tl.where(off_r == i, 1.0, v)
                ctile = tl.load(base_out + offs_f[:, None] * stride_c0 + off_r[None, :] * stride_c1,
                                mask=fmask[:, None] & rmask[None, :], other=0.0)
            ...
            while r < DIM:
                ...
                tl.store(base_out + offs_f[:, None] * stride_c0 + off_r[None, :] * stride_c1,
                         ctile, mask=fmask[:, None] & rmask[None, :])
                r += BLOCK_R
...
@triton.jit
def _wy_t_kernel(
    input_ptr, tau_ptr, t_ptr,
    M, K,
    stride_im, stride_ik, stride_ib, stride_tb,
    ...
    while r < M:
        roff = r + off_r
        rmask = roff < M
        v = tl.load(base_in + roff[:, None] * stride_im + (j0 + off_b)[None, :] * stride_ik,
    ...
    tl.store(t_ptr + pid_b * stride_tbb + blk * BS * BS + off_b[:, None] * BS + off_b[None, :], T)


@triton.jit
def _wy_apply_kernel(
    input_ptr, other_ptr, out_ptr, t_ptr, y_ptr,
    DIM, F, K, NBLK, NPROG,
    stride_im, stride_ik, stride_ib,
    stride_c0, stride_c1, stride_cb,
    stride_tbb,
    LEFT: tl.constexpr, ASCEND: tl.constexpr, TRANSPOSE: tl.constexpr,
    BS: tl.constexpr, BR: tl.constexpr, BC: tl.constexpr,
    IS_F64: tl.constexpr, USE_DOT: tl.constexpr,
):
    ...
    for r in range(0, DIM, BR):
        ...
        rmask = roff < DIM
        if LEFT:
            tile = tl.load(base_oth + roff[:, None] * stride_c0 + offs_f[None, :] * stride_c1,
                           mask=rmask[:, None] & fmask[None, :], other=0.0)
            tl.store(base_out + roff[:, None] * stride_c0 + offs_f[None, :] * stride_c1,
                     tile, mask=rmask[:, None] & fmask[None, :])
        else:
            tile = tl.load(base_oth + offs_f[:, None] * stride_c0 + roff[None, :] * stride_c1,
                           mask=fmask[:, None] & rmask[None, :], other=0.0)
            tl.store(base_out + offs_f[:, None] * stride_c0 + roff[None, :] * stride_c1,
                     tile, mask=fmask[:, None] & rmask[None, :])

    if ASCEND:
        blk = 0
        while blk < NBLK:
            _wy_block(base_in, base_out, base_t, base_y, DIM, K, stride_im, stride_ik,
                      stride_c0, stride_c1, offs_f, fmask, off_r, off_b, blk,
                      LEFT, TRANSPOSE, BS, BR, BC, IS_F64, USE_DOT)
            blk += 1
    else:
        blk = NBLK - 1
        while blk >= 0:
...
@triton.jit
def _wy_block(
    base_in, base_out, base_t, base_y, DIM, K,
    stride_im, stride_ik, stride_c0, stride_c1,
    offs_f, fmask, off_r, off_b, blk,
    LEFT: tl.constexpr, TRANSPOSE: tl.constexpr,
    BS: tl.constexpr, BR: tl.constexpr, BC: tl.constexpr,
    IS_F64: tl.constexpr, USE_DOT: tl.constexpr,
):
    acc = tl.float64 if IS_F64 else tl.float32
    ...
    if IS_F64 and not USE_DOT:
        ...
        while r < DIM:
            ...
            rmask = roff < DIM
            if LEFT:
                ctile = tl.load(base_out + roff[:, None] * stride_c0 + offs_f[None, :] * stride_c1,
                                mask=rmask[:, None] & fmask[None, :], other=0.0)
            else:
                ctile = tl.load(base_out + offs_f[:, None] * stride_c0 + roff[None, :] * stride_c1,
                                mask=fmask[:, None] & rmask[None, :], other=0.0)
            for b in range(0, BS):
                v_b = tl.load(base_in + roff * stride_im + (j0 + b) * stride_ik, mask=rmask, other=0.0)
        ...
        if LEFT:
            for i in range(0, BS):
                if TRANSPOSE:
                    t_i = tl.load(base_t + blk * BS * BS + off_b * BS + i, mask=off_b < bse, other=0.0)
                else:
                    t_i = tl.load(base_t + blk * BS * BS + i * BS + off_b, mask=off_b < bse, other=0.0)
    ...
    if LEFT:
        ...
        while r < DIM:
            roff = r + off_r
            rmask = roff < DIM
            v = tl.load(vptr + roff[:, None] * stride_im, mask=rmask[:, None], other=0.0)
            v = tl.where(roff[:, None] < (j0 + off_b)[None, :], 0.0, v)
            v = tl.where(roff[:, None] == (j0 + off_b)[None, :], 1.0, v)
            v = tl.where(off_b[None, :] < bse, v, 0.0)
            ctile = tl.load(base_out + roff[:, None] * stride_c0 + offs_f[None, :] * stride_c1,
...
def _launch_naive(input, tau, other, out, left, transpose, F, M, K, B,
                  stride_ib, stride_tb, stride_cb, stride_c0, stride_c1, IS_F64):
    if F <= 64:
        BLOCK_C = 64
    else:
        BLOCK_C = 128
    ...
    _ormqr_kernel[grid](
        input, tau, other, out, M, F, K,
        input.stride(-2), input.stride(-1), stride_ib, stride_tb,
        stride_c0, stride_c1, stride_cb,
        LEFT=left, ASCEND=ASCEND, BLOCK_R=BLOCK_R, BLOCK_C=BLOCK_C, IS_F64=IS_F64,
        num_warps=2,
    )


def _launch_wy(input, tau, other, out, left, transpose, F, M, K, B,
               stride_ib, stride_tb, stride_cb, stride_c0, stride_c1, IS_F64,
               BS=32, BR=32, BC=128, nw=4, use_dot=False):
    ...
    _wy_t_kernel[grid_t](
        ...
        input.stride(-2), input.stride(-1), stride_ib, stride_tb,
        stride_tbb,
        BS=BS, BR=BR, IS_F64=IS_F64,
        num_warps=2,
    )
    grid_a = (nprog, B)
    _wy_apply_kernel[grid_a](
        input, other, out, t_buf, y_buf, M, F, K, NBLK, nprog,
        input.stride(-2), input.stride(-1), stride_ib,
        stride_c0, stride_c1, stride_cb,
        stride_tbb,
        LEFT=left, ASCEND=ASCEND, TRANSPOSE=transpose,
        BS=BS, BR=BR, BC=BC, IS_F64=IS_F64, USE_DOT=use_dot,
        num_warps=nw,
    )
...
def run(input, tau, other, left=True, transpose=False):
    ...
    if IS_F64 and K >= 1024:
        if N <= 4096:
            ...
            _launch_wy(input, tau, other, out, left, transpose, F, M, K, B,
                       stride_ib, stride_tb, stride_cb, stride_c0, stride_c1, IS_F64,
                       BS=32, BR=32, BC=128, nw=8, use_dot=True)
        else:
            # fp64 elementwise WY path (tl.dot fp64 is slow at huge N); best for N>4096
            _launch_wy(input, tau, other, out, left, transpose, F, M, K, B,
                       stride_ib, stride_tb, stride_cb, stride_c0, stride_c1, IS_F64,
                       BS=64, BR=64, BC=64, nw=4)
    elif (not IS_F64) and K >= 512:
        # fp32 WY dot path, large-K tile config
        _launch_wy(input, tau, other, out, left, transpose, F, M, K, B,
                   stride_ib, stride_tb, stride_cb, stride_c0, stride_c1, IS_F64,
                   BS=64, BR=64, BC=128, nw=8)
    elif (not IS_F64) and K >= 48:
        # fp32 WY dot path, small/medium tile config
        _launch_wy(input, tau, other, out, left, transpose, F, M, K, B,
                   stride_ib, stride_tb, stride_cb, stride_c0, stride_c1, IS_F64,
                   BS=32, BR=32, BC=128, nw=4)
    else:
        _launch_naive(input, tau, other, out, left, transpose, F, M, K, B,
                      stride_ib, stride_tb, stride_cb, stride_c0, stride_c1, IS_F64)
    return out
