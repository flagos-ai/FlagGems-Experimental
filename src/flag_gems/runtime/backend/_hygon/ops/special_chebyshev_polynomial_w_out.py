...
def _cheb_w_scalar_n_kernel(
    ...
    N_IS_CONST: tl.constexpr, N_CONST: tl.constexpr,
    BLOCK: tl.constexpr,
):
    pid = tl.program_id(0)
...
def _cheb_w_tensor_n_kernel(
    x_ptr, n_ptr, out_ptr, numel,
    MAX_DEG: tl.constexpr, BLOCK: tl.constexpr,
):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < numel
...
def _launch_scalar(xf, n_src, of, numel, n_const, n_is_const):
    ...
    if numel % BLOCK == 0:
        ...
        _cheb_w_scalar_n_aligned_kernel[grid](
            xf, n_src, of, N_IS_CONST=n_is_const, N_CONST=n_const,
            BLOCK=BLOCK, num_warps=nw,
        )
    else:
        grid = (triton.cdiv(numel, BLOCK),)
        _cheb_w_scalar_n_kernel[grid](
            xf, n_src, of, numel, N_IS_CONST=n_is_const, N_CONST=n_const,
            BLOCK=BLOCK, num_warps=nw,
        )
