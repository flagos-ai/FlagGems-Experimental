...
def run(values, offsets, max_lengths, padding_value=0.0):
    ...
    if type(padding_value) is not float:
        padding_value = float(padding_value.item() if isinstance(padding_value, torch.Tensor) else padding_value)
    ...
    if ndim == 1:
        ...
        out = torch.empty((batch, max_len, inner), dtype=values.dtype, device=values.device)
