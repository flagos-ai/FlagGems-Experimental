...
@triton.jit
def _count_kernel(x_ptr, counts_ptr, n_elements,
                  BLOCK: tl.constexpr, SPLIT: tl.constexpr):
    pid = tl.program_id(0)
...
@triton.jit
def _single_kernel(x_ptr, outs_ptr, total_ptr, n_elements,
                   NDIM: tl.constexpr, ST0: tl.constexpr,
                   ST1: tl.constexpr, BLOCK: tl.constexpr):
...
@triton.jit
def _scatter_fused_kernel(x_ptr, outs_ptr, counts_ptr, n_blocks, n_elements,
                          NDIM: tl.constexpr, ST0: tl.constexpr,
                          ST1: tl.constexpr, BLOCK: tl.constexpr,
                          GB: tl.constexpr):
    pid = tl.program_id(0)
    ...
    cnt = tl.load(counts_ptr + offs_b, mask=offs_b < n_blocks, other=0) \
        .to(tl.int64)
    ...
    if NDIM == 1:
        ...
        tl.store(outs_ptr + 2 * n_elements + gpos, i2, mask=nz)
    if pid == n_blocks - 1:
        tl.store(counts_ptr + n_blocks,
                 (base + tl.sum(nz32).to(tl.int64)).to(tl.int32))


@triton.jit
def _chunkscan_kernel(counts_ptr, offsets_ptr, chunk_totals_ptr, n_blocks,
                      SB: tl.constexpr):
    c = tl.program_id(0)
...
@triton.jit
def _scatter_chunk_kernel(x_ptr, outs_ptr, offsets_ptr, chunk_totals_ptr,
                          counts_ptr, n_blocks, n_elements,
                          NDIM: tl.constexpr, ST0: tl.constexpr,
                          ST1: tl.constexpr, BLOCK: tl.constexpr,
                          SB: tl.constexpr, CB: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < n_elements
    x = tl.load(x_ptr + offs, mask=mask, other=0)
...
def run(inp):
    ...
    if n <= _SINGLE_N:
        ...
        _single_kernel[(1,)](x, outs, total, n, NDIM=ndim, ST0=st0, ST1=st1,
                             BLOCK=blk, num_warps=nw)
    ...
    outs = torch.empty((ndim, n), dtype=torch.int64, device=dev)
    _count_kernel[(G,)](x, counts, n, BLOCK=_BLOCK_C, SPLIT=_SPLIT,
                        num_warps=4)
    if ndim <= 3 and G4 <= _FOLD_G:
        _scatter_fused_kernel[(G4,)](x, outs, counts, G4, n, NDIM=ndim,
                                     ST0=st0, ST1=st1, BLOCK=_BLOCK_S,
                                     GB=_FOLD_G, num_warps=8)
    else:
        ...
        _chunkscan_kernel[(nchunks,)](counts, offsets, chunk_totals, G4,
                                      SB=_SB)
        _scatter_chunk_kernel[(G4,)](x, outs, offsets, chunk_totals, counts,
                                     G4, n, NDIM=ndim, ST0=st0, ST1=st1,
                                     BLOCK=_BLOCK_S, SB=_SB,
