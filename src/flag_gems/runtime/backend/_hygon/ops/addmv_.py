...
@triton.jit
def _addmv_kernel(
    mat_ptr, vec_ptr, partial_ptr, counter_ptr, self_ptr,
    M, N,
    stride_m, stride_k, stride_v, stride_self,
    beta, alpha,
    nrb,
...
def run(self, mat, vec, *, beta=1, alpha=1):
    ...
    if K_splits == 1:
        _addmv_kernel[grid](
            mat, vec, self, self, self,
            M, N,
            mat.stride(0), mat.stride(1), vec.stride(0), self.stride(0),
            beta, alpha,
            nrb,
            K_SPLITS=1, BM=BM, BK=BK, BN=BN, ACC_DTYPE=tl_acc,
        ...
        _addmv_kernel[grid](
            mat, vec, partial, counter, self,
            M, N,
            mat.stride(0), mat.stride(1), vec.stride(0), self.stride(0),
            beta, alpha,
            nrb,
            K_SPLITS=K_splits, BM=BM, BK=BK, BN=BN, ACC_DTYPE=tl_acc,
            num_warps=nw, num_stages=3,
        )
    return self
