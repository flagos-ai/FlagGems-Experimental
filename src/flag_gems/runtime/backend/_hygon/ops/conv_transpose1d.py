...
def get_hygon_configs():
    """
    Returns tuning configs optimized for Hygon DCU hardware.
    Focuses on memory coalescing and compute efficiency.
    """
    return [
        triton.Config({"BLOCK_N_OW": 16, "BLOCK_IC": 8, "BLOCK_OC": 16}, num_warps=2),
        triton.Config({"BLOCK_N_OW": 16, "BLOCK_IC": 16, "BLOCK_OC": 16}, num_warps=2),
        triton.Config({"BLOCK_N_OW": 32, "BLOCK_IC": 8, "BLOCK_OC": 16}, num_warps=2),
        triton.Config({"BLOCK_N_OW": 32, "BLOCK_IC": 16, "BLOCK_OC": 16}, num_warps=4),
        triton.Config({"BLOCK_N_OW": 32, "BLOCK_IC": 16, "BLOCK_OC": 32}, num_warps=4),
        triton.Config({"BLOCK_N_OW": 64, "BLOCK_IC": 8, "BLOCK_OC": 16}, num_warps=4),
        triton.Config({"BLOCK_N_OW": 64, "BLOCK_IC": 16, "BLOCK_OC": 16}, num_warps=4),
        triton.Config({"BLOCK_N_OW": 64, "BLOCK_IC": 16, "BLOCK_OC": 32}, num_warps=4),
        triton.Config({"BLOCK_N_OW": 128, "BLOCK_IC": 8, "BLOCK_OC": 16}, num_warps=4),
        triton.Config({"BLOCK_N_OW": 128, "BLOCK_IC": 16, "BLOCK_OC": 16}, num_warps=8),
        triton.Config({"BLOCK_N_OW": 256, "BLOCK_IC": 8, "BLOCK_OC": 16}, num_warps=8),
        triton.Config({"BLOCK_N_OW": 256, "BLOCK_IC": 16, "BLOCK_OC": 16}, num_warps=8),
    ]


@libentry()
@triton.autotune(
    configs=get_hygon_configs(),
    key=[
        "batch_size",
        "in_channels",
        "input_width",
...
@triton.jit
def conv_transpose1d_forward_kernel_hygon(
    input_pointer,
    weight_pointer,
    output_pointer,
    bias_pointer,
    ...
    """
    Hygon-optimized Triton kernel for 1D transposed convolution forward pass.

    Optimizations for Hygon DCU:
    - Tuned block sizes for better occupancy
    - Improved memory coalescing patterns
    - Optimized loop structure for reduced register pressure
    """
...
def conv_transpose1d(
    ...
    """
    Hygon-optimized 1D transposed convolution operator.
    ...
    """
    logger.debug("GEMS_HYGON CONV_TRANSPOSE1D")
    ...
    # Ensure contiguous tensors for better memory access patterns
    input_contig = input.contiguous()
    weight_contig = weight.contiguous()

    in_channels_per_group = in_channels // groups

    conv_transpose1d_forward_kernel_hygon[grid](
        input_contig,
        weight_contig,
        output,
        bias_pointer,
