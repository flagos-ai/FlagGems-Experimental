...
def _adaptive_avg_pool2d_bw_flat_div_kernel(
    ...
    if I64:
        if CONTIG:
            src = nc.to(tl.int64) * (H_out * W_out) + oh.to(tl.int64) * W_out + ow.to(tl.int64)
        else:
            n = nc // C
            c = nc % C
            src = (n.to(tl.int64) * s_n + c.to(tl.int64) * s_c
                   + oh.to(tl.int64) * s_h + ow.to(tl.int64) * s_w)
...
def run(grad_output, self):
    ...
    if s.dim() != 4 or g.dim() != 4:
        raise RuntimeError(
            "adaptive_avg_pool2d_backward expects 3D/4D tensors"
        )
    ...
    i64_total = (total >= (1 << 30)
                 or any(stride >= (1 << 31) for stride in g.stride()))
    ...
    if H_in % H_out == 0 and W_in % W_out == 0:
        ...
        _adaptive_avg_pool2d_bw_flat_div_kernel[grid](
            ...
            total,
            BLOCK=BLOCK, C=C, H_in=H_in, H_out=H_out,
            W_in=W_in, W_out=W_out,
            KH=kh, KW=kw, SHIFT8=(kh == 8 and kw == 8),
            I64=i64_total, CONTIG=contig, ACC_DTYPE=acc_dtype,
            num_warps=8,
        )
        ...
        _adaptive_avg_pool2d_bw_flat_dyn_kernel[grid](
            ...
            BLOCK=BLOCK, C=C, H_in=H_in, H_out=H_out,
            W_in=W_in, W_out=W_out,
            CONTIG=contig, ACC_DTYPE=acc_dtype,
            num_warps=8,
        )
    else:
        BLOCK = 1024
        ...
        _adaptive_avg_pool2d_bw_flat_kernel[grid](
            ...
            BLOCK=BLOCK, C=C, H_in=H_in, H_out=H_out,
            W_in=W_in, W_out=W_out,
            MAX_H=max_h, MAX_W=max_w, I64=i64,
            CONTIG=contig, ACC_DTYPE=acc_dtype,
            num_warps=4,
        )
