...
@triton.jit
def _mse_bwd_kernel(
    x_ptr, t_ptr, g_ptr, out_ptr, numel, scale,
    BLOCK: tl.constexpr, GSCALAR: tl.constexpr,
):
    pid = tl.program_id(0)
...
def run(grad_output, self, target, reduction=1):
    ...
    _mse_bwd_kernel[grid](
        self, target, grad_output, out, numel, scale,
        BLOCK=BLOCK, GSCALAR=g_scalar, num_warps=4,
    )
    return out
