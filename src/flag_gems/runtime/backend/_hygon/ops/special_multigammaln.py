...
def _lngamma_pos(x):
    ...
    z = x - 1.0
    z2 = z * z
    na = (12.08955101499484 * z + 355.1434057884724) * z2 + 2521.647135546938 * z + 6237.715489504789
    qa = ((z + 10.0) * z + 35.0) * z2 + 50.0 * z + 24.0
    nb = (12.36878141789309 * z + 259.882979235203) * z2 + 1807.920499474365 * z + 4163.669856507623
...
def _lg_low(x):
    ...
    u = x - 3.0
    return 0.693147183702582 + u * (0.9227845317179313 + u * (0.19746686226172916 + u * (-0.02568848062232315 + u * (0.004957270366521906 + u * (-0.0011246888096725022 + u * (0.0002820624569954515 + u * (-9.154338163163821e-05 + u * 2.6342403074772127e-05)))))))
...
def _lg_mid(x):
    ...
    u = x - 5.0
    return 3.1780538303663093 + u * (1.5061176704346058 + u * (0.11066147686213039 + u * (-0.008131651285415107 + u * (0.0008928348539137554 + u * (-0.00011708004428716894 + u * (1.700405112515271e-05 + u * (-2.8014995903583167e-06 + u * 4.5979572985597094e-07)))))))
...
def _lg_sum5(x):
    ...
    # max fp32 err ~1.5e-6.
    u = x - 3.5
    return 7.781670848155366 + u * (3.188372322871353 + u * (1.3976922344399867 + u * (-0.28859507326695955 + u * (0.09653109770791657 + u * (-0.040303088713329105 + u * (0.019261111106670586 + u * (-0.012104446484166531 + u * 0.006868100198728165)))))))
...
def _lgamma_stirling(x):
    ...
    r2 = r * r
    s = r * (0.08333333333333333 - r2 * (0.002777777777777778 - r2 * (0.0007936507936507937 - r2 * 0.0005952380952380953)))
    return (x - 0.5) * tl.log(x) - x + 0.9189385332046727 + s


@triton.jit
def _multigammaln_kernel(x_ptr, out_ptr, cadd, cmul, numel, BLOCK: tl.constexpr, P: tl.constexpr, J: tl.constexpr, ODD: tl.constexpr, PLAST: tl.constexpr, PSEC: tl.constexpr, LEFTOVER: tl.constexpr, MASKED: tl.constexpr, F64: tl.constexpr):
    pid = tl.program_id(0)
...
def run(self, p):
    ...
    _multigammaln_kernel[grid](xf, of, cadd, cmul, numel, BLOCK=BLOCK, P=p, J=p // 2, ODD=(p % 2 == 1), PLAST=p // 2 - 1, PSEC=p // 2 - 2, LEFTOVER=leftover, MASKED=masked, F64=f64, num_warps=warps)
    return out
