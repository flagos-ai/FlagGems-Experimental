...
def run(x, n):
    ...
    if isinstance(n, (int,)) and not isinstance(n, bool):
        _shifted_cheb_v_kernel[grid](
            x, x, out, n_elements, n,
            BLOCK=BLOCK, MAX_DEG=MAX_DEG,
            SCALAR_N=True, BROADCAST_N=False, EVEN=EVEN,
            num_warps=4,
        )
    elif torch.is_tensor(n) and n.numel() == 1:
        _shifted_cheb_v_kernel[grid](
            x, n, out, n_elements, 0,
            BLOCK=BLOCK, MAX_DEG=MAX_DEG,
            SCALAR_N=False, BROADCAST_N=True, EVEN=EVEN,
            num_warps=4,
        )
    else:
        _shifted_cheb_v_kernel[grid](
            x, n, out, n_elements, 0,
            BLOCK=BLOCK, MAX_DEG=MAX_DEG,
            SCALAR_N=False, BROADCAST_N=False, EVEN=EVEN,
            num_warps=4,
        )
    return out
