...
# Hygon DCU uses 64-lane wavefronts, so `num_warps=N` maps to N*64 threads.
# Fewer, wider warps keep this memory-bound 4x4-tap kernel efficient while a
# larger BLOCK_W amortizes the per-row y-weight setup across more columns.
@triton.autotune(
    configs=[
        triton.Config({"BLOCK_W": 256}, num_warps=2),
        triton.Config({"BLOCK_W": 256}, num_warps=4),
        triton.Config({"BLOCK_W": 512}, num_warps=4),
        triton.Config({"BLOCK_W": 512}, num_warps=8),
        triton.Config({"BLOCK_W": 1024}, num_warps=4),
        triton.Config({"BLOCK_W": 1024}, num_warps=8),
        triton.Config({"BLOCK_W": 2048}, num_warps=8),
    ],
    key=["W_out"],
)
@triton.jit
def _upsample_bicubic2d_row_kernel(
    ...
    # 64-bit addressing is only required for very large tensors; skipping it for
    # the common case shrinks the address arithmetic the DCU has to issue.
    if USE_INT32_IDX:
        n_idx = n
        c_idx = c
    else:
        n_idx = n.to(tl.int64)
        c_idx = c.to(tl.int64)
    base_ptr = in_ptr + n_idx * strideN + c_idx * strideC

    if USE_INT32_IDX:
        row_m1_ptr = base_ptr + y_m1 * strideH
        row_0_ptr = base_ptr + y_0 * strideH
        row_p1_ptr = base_ptr + y_p1 * strideH
        row_p2_ptr = base_ptr + y_p2 * strideH
    else:
    ...
    if USE_INT32_IDX:
        ...
        off_x_m1 = x_m1.to(tl.int64) * strideW
        off_x_0 = x_0.to(tl.int64) * strideW
        off_x_p1 = x_p1.to(tl.int64) * strideW
        off_x_p2 = x_p2.to(tl.int64) * strideW
    ...
    if USE_INT32_IDX:
        out_offset = n_idx * out_strideN + c_idx * out_strideC + y_out * out_strideH + x_out * out_strideW
    else:
        out_offset = (
            n_idx * out_strideN
            + c_idx * out_strideC
...
def upsample_bicubic2d(
    ...
    logger.debug("GEMS_HYGON UPSAMPLE_BICUBIC2D")
    ...
    if output_size is not None:
        if len(output_size) != 2:
            raise ValueError("output_size must be a sequence of two ints (H_out, W_out)")
    ...
    # 32-bit indices suffice when every input/output offset fits in int32; this
    # lets the DCU avoid 64-bit address math on the common (small) tensors.
    max_in_off = (N - 1) * sN + (C - 1) * sC + (H_in - 1) * sH + (W_in - 1) * sW
    max_out_off = N * C * H_out * W_out
    use_int32_idx = max(max_in_off, max_out_off) < 2**31 - 1
