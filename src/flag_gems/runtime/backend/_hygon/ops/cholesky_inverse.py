...
def _diag_inv_kernel(
    L_ptr, B_ptr,
    n, s0, s1, bs,
    NB: tl.constexpr, BLK: tl.constexpr, DT: tl.constexpr, USE_DOT: tl.constexpr,
):
    pid = tl.program_id(0)
    bid = tl.program_id(1)
    if pid * BLK >= n:
...
def _chain_kernel(
    ...
    for i in tl.range(j + 1, NB):
        ...
        for kp in tl.range(j, i):
            ...
            Lb = tl.load(
                lblock + (i * BLK + rows[:, None]) * s0 + (kp * BLK + cols[None, :]) * s1,
...
def _fused_small_kernel(
    L_ptr, out_ptr,
    n, s0, s1, bs,
    BLK: tl.constexpr, DT: tl.constexpr, USE_DOT: tl.constexpr,
):
...
def run(L, upper=False):
    ...
    if n <= 16:
        ...
        _fused_small_kernel[(batch,)](
            L, out, n, s0, s1, bs, BLK=BLK, DT=DT, USE_DOT=USE_DOT, num_warps=nw,
        )
        return out

    _diag_inv_kernel[(NB, batch)](
        L, B, n, s0, s1, bs, NB=NB, BLK=BLK, DT=DT, USE_DOT=USE_DOT,
    )
    if NB > 1:
    ...
    _ainv_kernel[(NB, NB, batch)](
        B, out, n, out.stride(-2), out.stride(-1), bs, NB=NB, BLK=BLK, DT=DT, USE_DOT=USE_DOT,
    )
    return out
