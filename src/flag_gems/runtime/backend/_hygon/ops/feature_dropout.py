...
def _feature_dropout_channel_kernel(
    X, Y,
    NC, SPATIAL,
    p, scale, philox_seed, philox_offset,
    BLOCK_C: tl.constexpr, BLOCK_S: tl.constexpr, SCALE2: tl.constexpr,
...
def _feature_dropout_elementwise_kernel(
    ...
    numel,
    p, scale, philox_seed, philox_offset,
    BLOCK: tl.constexpr, SCALE2: tl.constexpr, NO_MASK: tl.constexpr,
):
    ...
    r = tl.where(cols[None, :] == 0, r0[:, None],
                 tl.where(cols[None, :] == 1, r1[:, None],
...
def run(input, p, train=True):
    ...
    if spatial == 1:
        ...
        _feature_dropout_elementwise_kernel[grid](
            x, out, numel, p_f, scale, seed, offset,
            BLOCK=BLOCK, SCALE2=(scale == 2.0),
        ...
        _feature_dropout_channel_kernel[grid](
            x, out, NC, spatial, p_f, scale, seed, offset,
