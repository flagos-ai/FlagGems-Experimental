...
@triton.jit
def _pt_partial_minmax(x_ptr, min_ptr, max_ptr, n_elements,
...
@triton.jit
def _pt_finalize_qparams(min_ptr, max_ptr, rmin_ptr, rmax_ptr, scale_ptr,
                         zp_ptr, n_part, avg_c, qmin, qmax,
                         OBS: tl.constexpr, SYM: tl.constexpr, FQ: tl.constexpr,
                         BLOCK: tl.constexpr):
    ...
    for i in range(0, n_part, BLOCK):
        ...
        m = offs < n_part
        acc_min = tl.minimum(acc_min, tl.load(min_ptr + offs, mask=m, other=float("inf")))
        acc_max = tl.maximum(acc_max, tl.load(max_ptr + offs, mask=m, other=float("-inf")))
    ...
    if OBS:
        ...
        rmin = (rmin.to(tl.float64) * (1.0 - a64) + cmin.to(tl.float64) * a64).to(tl.float32)
        rmax = (rmax.to(tl.float64) * (1.0 - a64) + cmax.to(tl.float64) * a64).to(tl.float32)
...
@triton.jit
def _qparams_pc(cmin_ptr, cmax_ptr, rmin_ptr, rmax_ptr, rmin_out_ptr,
                rmax_out_ptr, scale_ptr, zp_ptr, C, avg_c, qmin, qmax,
                OBS: tl.constexpr, SYM: tl.constexpr, FQ: tl.constexpr,
                BLOCK: tl.constexpr):
...
@triton.jit
def _fq_pt(x_ptr, out_ptr, mask_ptr, scale_ptr, zp_ptr, N, qmin, qmax,
           BLOCK: tl.constexpr):
    pid = tl.program_id(0)
...
@triton.jit
def _fq_pc(x_ptr, out_ptr, mask_ptr, scale_ptr, zp_ptr, N, R, qmin, qmax,
           BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    off = pid * BLOCK + tl.arange(0, BLOCK)
    m = off < N
...
@triton.jit
def _fused_small_pc(x_ptr, out_ptr, mask_ptr, cmin_ptr, cmax_ptr,
                    rmin_ptr, rmax_ptr, scale_ptr, zp_ptr,
                    N, C, R, avg_c, qmin, qmax,
                    OBS: tl.constexpr, SYM: tl.constexpr, BLOCK: tl.constexpr):
    # Single-block fused path for small per-channel tensors: computes per-channel
    # min/max, observer update, qparams, and fake-quant in one launch.
    offs = tl.arange(0, BLOCK)
    # Phase 1: per-channel min/max
    for cc in range(0, C):
        lmin = tl.full([BLOCK], float("inf"), tl.float32)
        lmax = tl.full([BLOCK], float("-inf"), tl.float32)
        ...
        tl.store(cmin_ptr + cc, tl.min(lmin, axis=0))
        tl.store(cmax_ptr + cc, tl.max(lmax, axis=0))
    tl.debug_barrier()
    # Phase 2: qparams (vectorized over channels)
    cidx = offs
    m2 = cidx < C
    rmin = tl.load(rmin_ptr + cidx, mask=m2, other=0.0).to(tl.float32)
    rmax = tl.load(rmax_ptr + cidx, mask=m2, other=0.0).to(tl.float32)
    if OBS:
        cmin = tl.load(cmin_ptr + cidx, mask=m2, other=0.0).to(tl.float32)
        cmax = tl.load(cmax_ptr + cidx, mask=m2, other=0.0).to(tl.float32)
        a64 = avg_c.to(tl.float64)
        rmin = (rmin.to(tl.float64) * (1.0 - a64) + cmin.to(tl.float64) * a64).to(tl.float32)
        rmax = (rmax.to(tl.float64) * (1.0 - a64) + cmax.to(tl.float64) * a64).to(tl.float32)
        tl.store(rmin_ptr + cidx, rmin, mask=m2)
        tl.store(rmax_ptr + cidx, rmax, mask=m2)
    qmnf = qmin.to(tl.float32)
    qmxf = qmax.to(tl.float32)
    mn = tl.minimum(rmin, 0.0)
    mx = tl.maximum(rmax, 0.0)
    ...
    if SYM:
        ...
        sc = tl.where(both, sc_sym, sc)
        z = tl.where(both, z_sym, z)
    tl.store(scale_ptr + cidx, sc, mask=m2)
    tl.store(zp_ptr + cidx, z.to(tl.int32), mask=m2)
    tl.debug_barrier()
    # Phase 3: fake-quant
...
@triton.jit
def _fused_small_pc(x_ptr, out_ptr, mask_ptr, rmin_ptr, rmax_ptr,
                    scale_ptr, zp_ptr, N, C, R, avg_c, qmin, qmax,
                    OBS: tl.constexpr, SYM: tl.constexpr,
                    BLOCK_R: tl.constexpr, BLOCK: tl.constexpr):
    ...
    for cc in range(0, C):
        ...
        if OBS:
            rmin = (rmin.to(tl.float64) * (1.0 - a64) + cmin_s.to(tl.float64) * a64).to(tl.float32)
            rmax = (rmax.to(tl.float64) * (1.0 - a64) + cmax_s.to(tl.float64) * a64).to(tl.float32)
        ...
        if SYM:
            both = (mn < 0.0) & (mx > 0.0)
            sc_sym = tl.maximum((-mn.to(tl.float64)) / (-qmn64),
                                mx.to(tl.float64) / qmx64).to(tl.float32)
            sc_sym = tl.where(sc_sym == 0.0, 0.1, sc_sym)
            z_sym = _round_even((qmnf + qmxf) * 0.5)
            sc = tl.where(both, sc_sym, sc)
            z = tl.where(both, z_sym, z)
...
def run(self, observer_on, fake_quant_on, running_min, running_max, scale,
        zero_point, averaging_const, quant_min, quant_max, ch_axis,
        per_row_fake_quant=False, symmetric_quant=False):
    x = self
    if x.dtype is not torch.float32:
        name = {torch.float16: "Half", torch.bfloat16: "BFloat16",
                torch.float64: "Double"}.get(x.dtype, str(x.dtype))
    ...
    if not fq:
        if obs:
            if pc:
                ...
                _reduce_minmax[(triton.cdiv(R, RED_BLOCK), C)](xf, cmin, cmax, R, BLOCK=RED_BLOCK)
                _qparams_pc[(triton.cdiv(C, 128),)](cmin, cmax, running_min, running_max,
                                                    running_min, running_max, scale, zero_point,
                                                    C, ac, qmin, qmax,
                                                    OBS=True, SYM=sym, FQ=False, BLOCK=128)
            else:
                ...
                _pt_finalize_qparams[(1,)](mn, mx, running_min, running_max,
                                           scale, zero_point, n_part, ac, qmin, qmax,
                                           OBS=True, SYM=sym, FQ=False, BLOCK=RED_BLOCK)
    ...
    if obs and pc and fq and N <= 131072 and C <= 1024:
        ...
        _fused_small_pc[(1,)](xf, out, mask,
                              running_min, running_max, scale, zero_point,
                              N, C, R, ac, qmin, qmax,
                              OBS=True, SYM=sym, BLOCK_R=blk_r, BLOCK=1024)
    ...
    if obs:
        if pc:
            ...
            _qparams_pc[(triton.cdiv(C, 128),)](cmin, cmax, running_min, running_max,
                                                running_min, running_max, scale, zero_point,
                                                C, ac, qmin, qmax,
                                                OBS=True, SYM=sym, FQ=True, BLOCK=128)
        else:
            n_part = triton.cdiv(N, CHUNK)
            mn = torch.empty(n_part, dtype=torch.float32, device=dev)
            mx = torch.empty(n_part, dtype=torch.float32, device=dev)
            ...
            _pt_finalize_qparams[(1,)](mn, mx, running_min, running_max,
                                       scale, zero_point, n_part, ac, qmin, qmax,
                                       OBS=True, SYM=sym, FQ=True, BLOCK=RED_BLOCK)
    else:
        if pc:
            _qparams_pc[(triton.cdiv(C, 128),)](running_min, running_max,
                                                running_min, running_max,
                                                running_min, running_max,
                                                scale, zero_point,
                                                C, ac, qmin, qmax,
                                                OBS=False, SYM=sym, FQ=True, BLOCK=128)
        else:
            _pt_finalize_qparams[(1,)](xf, xf, running_min, running_max,
                                       scale, zero_point, 0, ac, qmin, qmax,
                                       OBS=False, SYM=sym, FQ=True, BLOCK=RED_BLOCK)
