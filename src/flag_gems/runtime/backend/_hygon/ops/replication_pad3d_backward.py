...
def _rep_pad3d_bwd_fused(
    ...
    for dd in tl.range(d_lo, d_hi + 1):
        ...
        lt = tl.load(rowb + ls_idx[None, :],
                     mask=hmask & (ls_idx[None, :] < n_ls),
                     other=0.0).to(DT)
        ls = tl.sum(lt, axis=1)  # [BLOCK_H]
        rt = tl.load(rowb + (rs_start + rs_idx)[None, :],
                     mask=hmask & (rs_idx[None, :] < n_rs),
                     other=0.0).to(DT)
        rs = tl.sum(rt, axis=1)  # [BLOCK_H]
        sv = tl.load(rowb + (w + left)[None, :],
                     mask=hmask & wmask_t[None, :] & ((w + left)[None, :] >= 0) & ((w + left)[None, :] < W_out),
                     other=0.0).to(DT)
        c = tl.where(w[None, :] == 0, ls[:, None],
                     tl.where(w[None, :] == W - 1, rs[:, None], sv))
        c = tl.where((W == 1) & (w[None, :] == 0), ls[:, None] + rs[:, None] - sv, c)
        acc += c

    outb = out + pid_plane * out_plane_stride + d * (H * W)
    tl.store(outb + h[:, None] * W + w[None, :], acc.to(ODT),
             mask=hmask & wmask_t[None, :])
    ...
    if h0 == 0:
        ...
        for dd in tl.range(d_lo, d_hi + 1):
            ...
            rt = tl.load(rowb + (rs_start + rs_idx)[None, :],
                         ...
                         other=0.0).to(DT)
            rs0 = tl.sum(rt)
            svt = tl.load(rowb + (w + left)[None, :],
                          mask=hhm & wmask_t[None, :]
                          & ((w + left)[None, :] >= 0)
                          & ((w + left)[None, :] < W_out),
                          other=0.0).to(DT)
...
def run(grad_output, self, padding):
    ...
    if self.dtype not in (torch.float16, torch.bfloat16, torch.float32):
        raise ValueError("replication_pad3d_backward supports float16, bfloat16, and float32")
