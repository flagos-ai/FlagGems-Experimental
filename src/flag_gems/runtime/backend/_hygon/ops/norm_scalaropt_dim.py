...
def _norm_lastdim_kernel(
    X, Out, M, N, p,
    IS_P2: tl.constexpr, IS_P1: tl.constexpr, IS_P0: tl.constexpr,
    IS_PINF: tl.constexpr, IS_PNINF: tl.constexpr, ACC_F64: tl.constexpr,
    ...
    if IS_PINF:
        ...
        for k0 in range(0, N, BLOCK_N):
            ...
            a = tl.load(X + rows[:, None] * N + cols[None, :], mask=mask, other=0.0).to(acc_dtype)
        ...
        for k0 in range(0, N, BLOCK_N):
            cols = k0 + tl.arange(0, BLOCK_N)
            mask = rmask[:, None] & (cols < N)[None, :]
            a = tl.load(X + rows[:, None] * N + cols[None, :], mask=mask, other=float("inf")).to(acc_dtype)
...
def _norm_lastdim_combine_kernel(
    Mid, Out, M, NSPLIT, p,
    IS_P2: tl.constexpr, IS_P1: tl.constexpr, IS_P0: tl.constexpr,
    IS_PINF: tl.constexpr, IS_PNINF: tl.constexpr,
    BLOCK_M: tl.constexpr, BLOCK_SP: tl.constexpr,
):
    pid = tl.program_id(0)
    rows = pid * BLOCK_M + tl.arange(0, BLOCK_M)
    sp = tl.arange(0, BLOCK_SP)
    if IS_PINF:
        mid = tl.load(Mid + sp[None, :] * M + rows[:, None],
                      mask=(rows < M)[:, None] & (sp < NSPLIT)[None, :], other=float("-inf"))
        ...
        mid = tl.load(Mid + sp[None, :] * M + rows[:, None],
                      mask=(rows < M)[:, None] & (sp < NSPLIT)[None, :], other=0.0)
...
def _norm_strided_kernel(
    ...
    IS_P2: tl.constexpr, IS_P1: tl.constexpr, IS_P0: tl.constexpr,
    IS_PINF: tl.constexpr, IS_PNINF: tl.constexpr, ACC_F64: tl.constexpr,
    BLOCK_MO: tl.constexpr, BLOCK_S: tl.constexpr,
):
    acc_dtype = tl.float64 if ACC_F64 else tl.float32
    ...
    if IS_PINF:
        ...
        for k0 in range(0, N):
            a = tl.load(X + base + k0 * S, mask=tile_mask, other=float("inf")).to(acc_dtype)
...
def _single_dim(x, p, d, keepdim, ndim, shape, acc_f64):
    ...
    use_f64 = acc_f64 or (
        ...
        and (N >= 8192 if not is_strided else True)
    )
    ...
    if S == 1:
        ...
        _norm_strided_kernel[grid](
            x, out, M_outer, N, S, p,
            is_p2, is_p1, is_p0, is_pinf, is_pninf, use_f64,
            BLOCK_MO=_BLOCK_MO, BLOCK_S=_BLOCK_S,
        )
    return out
...
def run(x, p, dim, keepdim=False):
    ...
    if len(dims) > 1:
        ...
        for i, d in enumerate(reversed(dims)):
            ...
            cur = _single_dim(cur, pp, d, True, cur.ndim, cur_shape,
                              cur.dtype == torch.float64)
