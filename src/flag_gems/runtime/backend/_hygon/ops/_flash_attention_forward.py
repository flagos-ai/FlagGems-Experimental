...
def _flash_attn_kernel(
    ...
    q_ptrs = Q + q_rows[:, None] * stride_qb + pid_h * stride_qh + d_offs[None, :] * stride_qd
    if EVEN_D:
    ...
    for kb in range(kb_start, kb_end + 1):
        ...
        k_ptrs = K + k_rows[None, :] * stride_kb + kv_head * stride_kh + d_offs[:, None] * stride_kd
        v_ptrs = V + k_rows[None, :] * stride_vb + kv_head * stride_vh + d_offs[:, None] * stride_vd
        if EVEN_D:
        ...
        if window_left >= 0 and window_right >= 0 and window_left >= seqlen_k and window_right >= seqlen_k:
            pass
        else:
            if window_left >= 0:
                mask2d = mask2d & (n_offs[None, :] >= m_offs[:, None] + off - tl.minimum(window_left, seqlen_k))
            if window_right >= 0:
                mask2d = mask2d & (n_offs[None, :] <= m_offs[:, None] + off + tl.minimum(window_right, seqlen_k))
        if HAS_SEQUSED:
        ...
        if HAS_DROPOUT:
            ...
            offs2d = (batch_po + m_offs[:, None] * seqlen_k + n_offs[None, :]).to(tl.uint32)
    ...
    out_ptrs = Out + q_rows[:, None] * stride_ob + pid_h * stride_oh + d_offs[None, :] * stride_od
    if EVEN_D:
        tl.store(out_ptrs, out.to(Out.dtype.element_ty), mask=m_mask[:, None])
    else:
        tl.store(out_ptrs, out.to(Out.dtype.element_ty), mask=m_mask[:, None] & d_mask[None, :])
...
def run(query, key, value, cumulative_sequence_length_q, cumulative_sequence_length_k, max_q, max_k, dropout_p, is_causal, return_debug_mask, *, scale=None, window_size_left=None, window_size_right=None, seqused_k=None, alibi_slopes=None):
