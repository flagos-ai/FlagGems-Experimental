...
def _shifted_cheb_u_direct(
    x_ptr, n_ptr, out_ptr, numel,
    X_IS_FP64: tl.constexpr, BLOCK: tl.constexpr,
):
    pid = tl.program_id(0)
...
def _shifted_cheb_u_bcast(
    x_ptr, n_ptr, out_ptr,
    dims_ptr, sx_ptr, sn_ptr, sp_ptr, numel,
    X_IS_FP64: tl.constexpr, BLOCK: tl.constexpr,
):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
...
def run(x, n):
    ...
    if simple:
        ...
        _shifted_cheb_u_direct[grid](
            x, n, out, numel, X_IS_FP64=x.dtype == torch.float64,
        ...
        _shifted_cheb_u_bcast[grid](
            x, n, out, dims_t, sx_t, sn_t, sp_t, numel,
            X_IS_FP64=x.dtype == torch.float64, BLOCK=_BLOCK, num_warps=_NUM_WARPS,
        )
    return out
