...
def _k_eigvals_full(
    ...
    for s in tl.range(0, 4 * N + 32, loop_unroll_factor=1):
        ...
        if nn >= 0:
            # deflation scan: largest l in [1, nn] with |a[l][l-1]| negligible
            sub = tl.load(
            ...
            l = tl.max(tl.where(cond & (ar >= 1) & (ar <= nn), ar, 0))
    ...
    if N == 10:
        ...
        o0i = tl.load(out_ptr + 1)
        o1r = tl.load(out_ptr + 2)
        o1i = tl.load(out_ptr + 3)
        o2r = tl.load(out_ptr + 4)
        o2i = tl.load(out_ptr + 5)
        o3r = tl.load(out_ptr + 6)
        o3i = tl.load(out_ptr + 7)
        o4r = tl.load(out_ptr + 8)
        o4i = tl.load(out_ptr + 9)
        o5r = tl.load(out_ptr + 10)
        o5i = tl.load(out_ptr + 11)
        o6r = tl.load(out_ptr + 12)
        o6i = tl.load(out_ptr + 13)
        o7r = tl.load(out_ptr + 14)
        o7i = tl.load(out_ptr + 15)
        o8r = tl.load(out_ptr + 16)
        o8i = tl.load(out_ptr + 17)
        o9r = tl.load(out_ptr + 18)
        o9i = tl.load(out_ptr + 19)
