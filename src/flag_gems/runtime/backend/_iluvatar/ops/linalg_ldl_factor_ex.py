...
def _ldl_kernel(
    ...
    if N <= 16:
        ...
        tl.store(ld_base + offs, tl.zeros((NP2, NP2), dtype=tl.float32),
        ...
        for k in tl.range(0, N, 2):
            ...
            mk = r >= k
            tl.store(ld_base + r * N + k,
                     tl.where(r == k, d1, lk), mask=mk & (r < N))
            mk1 = r >= k + 1
            tl.store(ld_base + r * N + (k + 1),
                     tl.where(r == k + 1, d2, lk1), mask=mk1 & (r < N))
        ...
        for k in tl.range(N):
            ...
            lcol = col / dk
            L = tl.where(c[None, :] == k,
                         tl.where(r[:, None] >= k, lcol[:, None], 0.0), L)
            D = tl.where(r == k, dk, D)
            act = act - col[:, None] * lcol[None, :]
        ld = tl.where(r[:, None] >= c[None, :],
                      tl.where(r[:, None] == c[None, :], D[None, :], L), 0.0)
