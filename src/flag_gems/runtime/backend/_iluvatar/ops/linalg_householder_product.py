...
_hhp_native = torch.linalg.householder_product


def _hhp_reference(A, tau):
    # This platform's cusolver lacks the fp64 Dorgqr path, so the fp64
    # reference is evaluated through the (working) CPU backend.  The fp32
    # reference path is untouched.
    if A.dtype == torch.float64:
        return _hhp_native(A.cpu(), tau.cpu()).to(A.device)
    return _hhp_native(A, tau)


torch.linalg.householder_product = _hhp_reference
