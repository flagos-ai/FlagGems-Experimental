...
Numerics: the corex backend's libdevice acos only accepts fp32, so fp16/bf16
...
(exact), and fp32->bf16 via pure truncation (ybits>>16), which is NaN-safe
(device acosf NaN payload 0x7FFFFFFF truncates to 0x7FFF, a bf16 NaN) and
lands within 1 ulp of the reference's round-to-nearest-even, well inside the
bf16 evaluation tolerance.
...
@triton.jit
def _arccos_inplace_fp16_kernel(x_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
...
def _arccos_inplace_bf16_kernel(x_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    ...
    # fp32 -> bf16 by truncation: NaN-safe, within 1 ulp of RNE
    yb = y.to(tl.uint32, bitcast=True) >> 16
...
def _arccos_inplace_bf16_kernel_nomask(x_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    ...
    f = (u.to(tl.uint32) << 16).to(tl.float32, bitcast=True)  # bf16 -> fp32 (exact)
    y = tl.extra.libdevice.acos(f)
    yb = y.to(tl.uint32, bitcast=True) >> 16  # truncation, NaN-safe
    tl.store(x_ptr + offsets, yb.to(tl.uint16).to(tl.bfloat16, bitcast=True))


def arccos_(x):
    xf = x.view(-1)  # zero-copy flat view (inputs are contiguous)
    n = xf.numel()
    if x.dtype == torch.float32:
        ...
        _arccos_inplace_fp16_kernel[(triton.cdiv(n, BLOCK_SIZE),)](
            xf, n, BLOCK_SIZE=BLOCK_SIZE, num_warps=NUM_WARPS
        )
    return x
