...
def _reduce_partial_mean_var_kernel(
    ...
    for chunk_idx in tl.range(0, num_chunks, BLOCK_SIZE):
        ...
        vals_s = tl.load(
            partial_sum_ptr + pid * num_chunks + chunk_offs, mask=chunk_mask, other=0.0
        )
        vals_q = tl.load(
            partial_sq_ptr + pid * num_chunks + chunk_offs, mask=chunk_mask, other=0.0
        )
...
def _reduce_partial_sum_kernel(
    ...
    for chunk_idx in tl.range(0, num_chunks, BLOCK_SIZE):
        ...
        chunk_mask = chunk_offs < num_chunks
        vals = tl.load(
            partial_in_ptr + pid * num_chunks + chunk_offs, mask=chunk_mask, other=0.0
        )
        acc += tl.sum(vals)
...
def var(x, dim=None, *, correction=None, keepdim=False):
    logger.debug("GEMS_ASCEND VAR_FORWARD")
